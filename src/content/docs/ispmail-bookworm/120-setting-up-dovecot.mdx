...
Also edit the “mail\_plugins” line to enable the *quota* plugin we will configure later, and turn it into:
...
- userdb_quota_rule  
   This is the user’s quota in bytes. Think of it as the maximum possible space on disk that the user can occupy. As [documented](https://doc.dovecot.org/configuration_manual/2.3/quota/#per-user-quota)
...
- userdb_uid and userdb_gid  
   Those are the user ID and group ID of the *vmail* user – 5000 for both. Dovecot uses it to set the permissions of files it
  creates. As all users share the same system user “vmail” this is just a static number.
